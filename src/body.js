...
`;
    ...
    const rootStyle = document.documentElement.style; // allows setting css variables
    ...
    const activateSaveButton = _ => {
      rootStyle.setProperty('--display-save', 'inline');
      // activate reminder about unsaved content
    ...
    changePasswordButton.addEventListener('click', async (_) => {
      ...
      // show the password selection ui
      form.password.value = '';
      form.repassword.value = '';
      rootStyle.setProperty('--display-form', 'inline');
      rootStyle.setProperty('--display-repw', 'inline');
      rootStyle.setProperty('--display-main', 'none');
      rootStyle.setProperty('--display-save', 'none');
      form.password.focus();
    });

    saveButton.addEventListener('click', async (_) => {
      // create a file of the source
      const fileContent = updateSourceData(source, await encrypt(currentPassword, dataInput.innerText));
    ...
    const showDecrypted = (decryptedData) => {
      ...
      rootStyle.setProperty('--display-form', 'none');
      rootStyle.setProperty('--display-repw', 'none');
      rootStyle.setProperty('--display-main', 'inline');

      // add text field for the data
      if (decryptedData !== undefined) { // undefined means that password was changed
        dataInput.innerText = decryptedData;
      }
      dataInput.removeEventListener('input', activateSaveButton, { once: true }); // remove if exists
      dataInput.addEventListener('input', activateSaveButton, { once: true });

      // detect ctrl+s, save only if changes
      window.addEventListener('keydown', (event) => {
        if (!(event.ctrlKey && event.key === 's')) return true;
        if (rootStyle.getPropertyValue('--display-save') === 'inline') {
          saveButton.click();
        }
