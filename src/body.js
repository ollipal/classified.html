...
if (typeof window === 'undefined') { // executed in Node.js
  ...
  */
  const question = (question, previous = []) => new Promise(resolve => {
    ...
    process.stdin.on('data', detectArrow);
    rl.question(question, (reply) => {
      process.stdin.removeListener('data', detectArrow);
      rl.close();
      resolve(reply);
    });
  });
  ...
  (async () => {
    // always clear the console on exit if exit has not been handled properly
    ...
    if (['-h', '--help', 'help'].includes(args[0])) {
      console.log(`classified.html version ${VERSION}
Open in browser or with Node.js by running 'node ${filename}'


Advanced usage: [COMMAND [TARGET [VALUE...]]]

COMMANDS:
  Data modifying commands:
    add         add VALUE to TARGET
    replace     replace existing value with VALUE on TARGET
    delete      delete TARGET and its value
    copy        copy TARGET's value to clipboard
    show        show TARGET's value on the terminal, clear the terminal after pressing enter and exit
...
Example usage:
  ...
  add value 'server key' 1234         add a new value '1234' with a key 'server key'
  copy value 'server key'             copy value with a key 'server key' to the clipboard
`);
      exitHandled = true;
      process.exit();
    }
    if (['-n', '--new', 'new'].includes(args[0])) {
    ...
    */
    const parseCommand = async (previous) => {
      const reply = await question("type 'help', enter a command or leave empty to exit: ", previous);
      let [command, target, ...value] = reply.split(' ');
      value = value.join(' ');
    ...
    while (true) {
      ...
      const [command, target, value] = await parseCommand(previous);
      ...
      // prepend list of hints if value was used, and is different to the first one
      if (!unknownCommand) {
